...
const { bot_token, bot_prefix, openAi_key } = JSON.parse(
...
`);
    ...
    bot.on("message", async (ctx) => {
      ...
      switch (command) {
        ...
        case "ig":
          ...
          let vid_url = args[0].then((res) => {
            ctx.replyWithMediaGroup(res.ur_list, {
    ...
    function logFunction(isGroup, isCmd, typeMessage, user, groupName) {
      if (!isGroup && !isCmd)
        console.log(
          chalk.whiteBright("├"),
          chalk.cyanBright("[ PRIVATE ]"),
          chalk.whiteBright(typeMessage),
          chalk.greenBright("from"),
          chalk.whiteBright(user.full_name)
        );
      if (isGroup && !isCmd)
        console.log(
          chalk.whiteBright("├"),
          chalk.cyanBright("[  GROUP  ]"),
          chalk.whiteBright(typeMessage),
          chalk.greenBright("from"),
          chalk.whiteBright(user.full_name),
